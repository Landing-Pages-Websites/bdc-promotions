...
function objectAt(input: JsonObject, path: readonly (string | number)[]): JsonObject {
...
function boundedContractStringCase(
  ...
  return {
    ...
    input: changed(managedSiteContract, (input) => {
      const key = path.at(-1);
      if (key === undefined) throw new Error("Bounded-string path must not be empty");
      objectAt(input, path.slice(0, -1))[key] = astralStringWithCodeUnits(length);
    }),
    valid,
  };
}
...
const CONTRACT_CASES: readonly DifferentialCase[] = [
    ...
    invalidContract("unknown root key", (input) => { input.extra = true; }),
    invalidContract("prototype-shaped root key", (input) => {
      Object.defineProperty(input, "__proto__", { enumerable: true, value: true });
    }),
    ...
    invalidContract("invalid stable ID", (input) => { input.contractId = "contract_home"; }),
    invalidContract("repository traversal", (input) => {
      objectAt(input, ["pages", 0, "sections", 0, "fields", 0, "resolver"]).path = "../secret.webp";
    }),
    invalidContract("URI-fragment JSON pointer", (input) => {
      objectAt(input, ["pages", 0, "sections", 0, "fields", 0, "resolver"]).pointer = "#/title";
    }),
    ...
    invalidContract("invalid fragment declaration", (input) => {
      objectAt(input, ["pages", 0, "sections", 0, "fields", 2, "constraints"]).allowedFragments = ["#admin"];
    }),
    ...
    invalidContract("alternate bridge source", (input) => {
      objectAt(input, ["bridge", "delivery"]).src = "https://evil.example/review-bridge.js";
    }),
    ...
    invalidContract("capability outside field type", (input) => {
      objectAt(input, ["pages", 0, "sections", 0, "fields", 0]).capabilities = ["image.upload"];
    }),
    ...
    invalidContract("duplicate rich policy", (input) => {
      objectAt(input, ["pages", 0, "sections", 0, "fields", 1, "constraints"]).allowedTargets = ["same_window", "same_window"];
    }),
    invalidContract("link label range exceeds content maximum", (input) => {
      objectAt(input, ["pages", 0, "sections", 0, "fields", 2, "constraints", "labelConstraints"]).maxLength = 2_001;
    }),
    invalidContract("forbidden fragment declaration", (input) => {
      const constraints = objectAt(input, ["pages", 0, "sections", 0, "fields", 2, "constraints"]);
    ...
    invalidContract("duplicate equivalent aspect ratios", (input) => {
      objectAt(input, ["assets", 0]).aspectRatios = [{ width: 2, height: 1 }, { width: 4, height: 2 }];
    }),
    ...
    invalidContract("duplicate collection uniqueness key", (input) => {
      objectAt(input, ["collections", 0, "uniqueness", 0]).fieldIds = [stableId("field"), stableId("field")];
    }),
    invalidContract("presentation example exceeds depth eight", (input) => {
      objectAt(input, ["pages", 0, "presentation"]).example = nestedValue(9);
    }),
    invalidContract("prototype-shaped key cannot hide presentation depth", (input) => {
      objectAt(input, ["pages", 0, "presentation"]).example =
        objectWithOwnKey("__proto__", nestedValue(8));
    }),
    ...
    invalidContract("invalid SEO outline level", (input) => {
      objectAt(input, ["internalSeo", "pages", 0, "headingOutline", 0]).semanticLevel = 7;
    }),
...
function validContentCases(): readonly DifferentialCase[] {
  return [
    ...
    {
      name: "heading value",
      input: () => contentWithValue({
        fieldId: stableId("field"), owner: { kind: "site" }, type: "heading_text", value: "Heading",
      }),
      valid: true,
    },
    { name: "rich-text value", input: () => contentWithValue(richTextContentValue("https://example.com/path")), valid: true },
    { name: "link value", input: () => contentWithValue(linkContentValue({ label: "Call", destination: { kind: "phone", number: "+14165551234" }, target: "same_window" })), valid: true },
    { name: "image value", input: () => contentWithValue({ fieldId: stableId("field"), owner: { kind: "site" }, type: "image", value: imageValue() }), valid: true },
    { name: "collection value", input: () => contentWithValue({ fieldId: stableId("field"), owner: { kind: "site" }, type: "collection", value: { orderedItemIds: [stableId("item")] } }), valid: true },
    { name: "postal address", input: () => contentWithValue(internalValue("postal_address", { streetAddress: "1 Main St", addressLocality: "Toronto", addressRegion: "ON", postalCode: "M5V 2T6", addressCountry: "CA" })), valid: true },
    { name: "geo coordinates", input: () => contentWithValue(internalValue("geo_coordinates", { latitude: 43.65, longitude: -79.38 })), valid: true },
    { name: "opening hours", input: () => contentWithValue(internalValue("opening_hours", { timeZone: "America/Toronto", periods: [{ days: ["monday"], allDay: false, opens: "09:00", closes: "17:00" }] })), valid: true },
    { name: "indexing directives", input: () => contentWithValue(internalValue("indexing_directives", { index: true, follow: true, archive: true, imageIndex: true, maxSnippet: -1, maxImagePreview: "large", maxVideoPreview: -1 })), valid: true },
    { name: "opaque JSON", input: () => contentWithValue(internalValue("json", { nested: [true, null, "copy"] })), valid: true },
    { name: "prototype-shaped opaque JSON key", input: () => contentWithValue(internalValue("json", objectWithOwnKey("__proto__", { safe: true }))), valid: true },
  ];
}

const INVALID_CONTENT_CASES: readonly DifferentialCase[] = [
    invalidContent("unknown document key", (input) => { input.extra = true; }),
    invalidContent("unknown value key", (input) => { objectAt(input, ["values", 0]).extra = true; }),
    invalidContent("wrong value discriminant", (input) => { objectAt(input, ["values", 0]).type = "html"; }),
    invalidContent("invalid field ID", (input) => { objectAt(input, ["values", 0]).fieldId = "field_home"; }),
    invalidContent("unsafe manifest path", (input) => { objectAt(input, ["assetManifest", 0]).path = "../hero.webp"; }),
    ...
    invalidContent("link label overflow", (input) => {
      input.values = [linkContentValue({ label: "x".repeat(2_001), destination: { kind: "email", address: "hello@example.com" }, target: "same_window" })];
    }),
    invalidContent("link label astral overflow", (input) => {
      input.values = [linkContentValue({ label: astralStringWithCodeUnits(2_001), destination: { kind: "email", address: "hello@example.com" }, target: "same_window" })];
    }),
    ...
    invalidContent("internal string-list item astral overflow", (input) => {
      input.values = [internalValue("string_list", [astralStringWithCodeUnits(2_049)])];
    }),
    invalidContent("postal-address string astral overflow", (input) => {
      input.values = [internalValue("postal_address", {
    ...
    invalidContent("image alt astral overflow", (input) => {
      ...
      input.values = [{ fieldId: stableId("field"), owner: { kind: "site" }, type: "image", value: image }];
    }),
    ...
    invalidContent("postal address extra key", (input) => {
      input.values = [internalValue("postal_address", { streetAddress: "1 Main St", addressLocality: "Toronto", addressRegion: "ON", postalCode: "M5V 2T6", addressCountry: "CA", district: "Downtown" })];
    }),
    invalidContent("latitude outside range", (input) => {
      input.values = [internalValue("geo_coordinates", { latitude: 91, longitude: 0 })];
    }),
    invalidContent("unknown IANA time zone", (input) => {
      input.values = [internalValue("opening_hours", { timeZone: "Mars/Olympus", periods: [] })];
    }),
    invalidContent("overlapping opening-hour days", (input) => {
      input.values = [internalValue("opening_hours", { timeZone: "UTC", periods: [{ days: ["monday"], allDay: true, opens: null, closes: null }, { days: ["monday"], allDay: false, opens: "09:00", closes: "17:00" }] })];
    }),
    invalidContent("invalid indexing bound", (input) => {
      input.values = [internalValue("indexing_directives", { index: true, follow: true, archive: true, imageIndex: true, maxSnippet: 10_001, maxImagePreview: "large", maxVideoPreview: -1 })];
    }),
];

function runtimeAccepts(parse: (input: unknown) => unknown, input: unknown): boolean {
  try {
    parse(input);
    return true;
  } catch {
...
describe("JSON Schema differential corpus", () => {
  it("matches the canonical contract parser across every C2-local class", () => {
    for (const candidate of CONTRACT_CASES) {
      ...
      assert.equal(runtimeAccepts(parseManagedSiteContractV1, input), candidate.valid, candidate.name);
      assert.equal(validateManagedSiteContractV1JsonSchema(input).valid, candidate.valid, candidate.name);
    }
  });

  it("matches the canonical content parser across every C2-local value class", () => {
    for (const candidate of [...validContentCases(), ...INVALID_CONTENT_CASES]) {
      const input = candidate.input();
      assert.equal(runtimeAccepts(parseManagedSiteContentDocument, input), candidate.valid, candidate.name);
      assert.equal(validateManagedSiteContentDocumentJsonSchema(input).valid, candidate.valid, candidate.name);
    }
  });
});
