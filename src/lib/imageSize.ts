/**
 * Reads the pixel dimensions of an image the site serves itself, straight from
 * the file header, with no dependency and no full decode.
 *
 ...
 *
 * Every failure path returns `null`. Callers must fall back to their own fixed
 * box, so an unreadable or unrecognised file is never worse than not looking.
 */
...
type ReadBytes = (offset: number, length: number) => Buffer | null;
...
function parse(read: ReadBytes, fileSize: number): ImageSize | null {
  const magic = read(0, 12);
  if (!magic) return null;
  if (magic.toString("latin1", 0, 8) === "\x89PNG\r\n\x1a\n") return parsePng(read);
  if (magic.toString("latin1", 0, 3) === "GIF") return parseGif(read);
  if (magic.toString("latin1", 0, 2) === "BM") return parseBmp(read);
  if (magic.readUInt8(0) === 0xff && magic.readUInt8(1) === 0xd8) {
    return parseJpeg(read, fileSize);
  }
  if (
    magic.toString("latin1", 0, 4) === "RIFF" &&
    magic.toString("latin1", 8, 12) === "WEBP"
  ) {
    return parseWebp(read);
  }
  if (magic.toString("latin1", 4, 8) === "ftyp") return parseAvif(read, fileSize);
  return null;
}
